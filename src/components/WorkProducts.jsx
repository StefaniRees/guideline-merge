...
export default function WorkProducts() {
  ...
  const roleColors = {
    ...
    [isEN ? 'Mediator' : 'Mediador']: { color: '#7F77DD', bg: '#EEEDFE', border: '#AFA9EC', href: '/guideline-merge/docs/roles/mediador' },
  ...
  const categories = [
    {
      ...
      artifacts: [
        { name: isEN ? 'Activity Branch' : 'Branch de Atividade', href: '/guideline-merge/docs/artifacts/branch-atividade', source: 'VCS', roles: ['Developer A', 'Developer B'], relations: isEN ? ['Standardized Commits', 'Pull Request', 'Conflict History'] : ['Commits Padronizados', 'Pull Request', 'Histórico de Conflitos'], desc: isEN ? 'Branch created from main to implement a change in an isolated and traceable way.' : 'Branch criada a partir da main para implementar uma mudança de forma isolada e rastreável.' },
        { name: isEN ? 'Standardized Commits' : 'Commits Padronizados', href: '/guideline-merge/docs/artifacts/commits-padronizados', source: 'VCS', roles: ['Developer A', 'Developer B'], relations: ['Pull Request', isEN ? 'Build/CI Report' : 'Relatório de Build/CI', 'Code Review'], desc: isEN ? 'Small, atomic commits with semantic messages and issue reference.' : 'Commits pequenos e atômicos com mensagens semânticas e referência à issue.' },
        { name: 'Pull Request (PR)', href: '/guideline-merge/docs/artifacts/pull-request', source: 'GitHub / GitLab / Azure Repos', roles: ['Developer A', 'Developer B', isEN ? 'Mediator' : 'Mediador'], relations: [isEN ? 'Review Checklist' : 'Checklist de Revisão', isEN ? 'Build/CI Report' : 'Relatório de Build/CI', isEN ? 'Decision Record' : 'Registro de Decisão'], desc: isEN ? 'Formal integration request with context, evidence, and automated validations.' : 'Solicitação formal de integração com contexto, evidências e validações automáticas.' },
        { name: isEN ? 'Review Checklist' : 'Checklist de Revisão', href: '/guideline-merge/docs/artifacts/checklist-revisao', source: isEN ? 'Docs / Repository' : 'Docs / Repositório', roles: [isEN ? 'Mediator' : 'Mediador', 'Developer B'], relations: ['Pull Request', 'Approval Policy', isEN ? 'Decision Record' : 'Registro de Decisão'], desc: isEN ? 'Objective guide for technical approval and review pending items.' : 'Guia objetivo para aprovação técnica e registro de pendências de revisão.' },
        { name: isEN ? 'Conflict History' : 'Histórico de Conflitos', href: '/guideline-merge/docs/artifacts/historico-conflitos', source: 'VCS / CI', roles: ['Developer A', 'Developer B', isEN ? 'Mediator' : 'Mediador'], relations: ['Pull Request', 'Commits', isEN ? 'Decision Record' : 'Registro de Decisão'], desc: isEN ? 'Conflict resolution trail, including causes, decisions, and lessons learned.' : 'Trilha de resolução de conflitos, incluindo causas, decisões e lições aprendidas.' },
      ],
    },
    {
      ...
      artifacts: [
        { name: isEN ? 'QA Report' : 'Relatório de QA', href: '/guideline-merge/docs/artifacts/relatorio-qa', source: 'CI / Test Runner', roles: ['Developer A', 'Developer B'], relations: [isEN ? 'Build/CI Report' : 'Relatório de Build/CI', 'Quality Gates', 'Pull Request'], desc: isEN ? 'Consolidation of functional and regression test evidence in staging.' : 'Consolidação de evidências de testes funcionais e de regressão em homologação.' },
        { name: isEN ? 'Build/CI Report' : 'Relatório de Build/CI', href: '/guideline-merge/docs/artifacts/relatorio-build-ci', source: 'CI (Actions, Pipelines)', roles: [isEN ? 'Automated pipeline' : 'Pipeline automático'], relations: ['Pull Request', isEN ? 'QA Report' : 'Relatório de QA', 'Deploy'], desc: isEN ? 'Automatic evidence of build, tests, and checks — technical gate before integration.' : 'Evidência automática de build, testes e verificações — gate técnico antes da integração.' },
      ],
    },
    {
    ...
    },
    {
      ...
      artifacts: [
        { name: isEN ? 'Decision Record' : 'Registro de Decisão', href: '/guideline-merge/docs/artifacts/registro-decisao', source: isEN ? 'PRs / Issues / Comments' : 'PRs / Issues / Comentários', roles: [isEN ? 'Mediator' : 'Mediador', 'Developer A', 'Developer B'], relations: ['Pull Request', 'Code Review', isEN ? 'Conflict History' : 'Histórico de Conflitos'], desc: isEN ? 'Technical justifications documented in PRs and reviews, supporting governance and auditing.' : 'Justificativas técnicas documentadas em PRs e revisões, apoiando governança e auditoria.' },
  ...
  const summaryStats = [
    { n: '10', label: isEN ? 'Artifacts' : 'Artefatos', href: '/guideline-merge/docs/artifacts', color: '#1D9E75', bg: '#E1F5EE' },
    { n: '8',  label: isEN ? 'Activities' : 'Atividades', href: '/guideline-merge/docs/activities', color: '#378ADD', bg: '#E6F1FB' },
    { n: '3',  label: isEN ? 'Roles' : 'Papéis', href: '/guideline-merge/docs/roles', color: '#7F77DD', bg: '#EEEDFE' },
    ...
    <div style={{ padding: '0 0 2rem' }}>
      ...
      <div style={{ background: '#fafafa', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '14px 16px', marginTop: '20px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
        ...
        <p style={{ fontSize: '13px', color: '#374151', lineHeight: 1.6, margin: 0 }}>
          {isEN
            ? <>Artifacts operationalize MergeTrace's principles of simplicity, transparency, and traceability. They enable explainability and audit mechanisms — identified by specialists (P1–P8) as essential for mitigating risks between teams, supporting{' '}<Link to="/guideline-merge/docs/welcome/roles" style={{ color: '#1D9E75', fontWeight: 600 }}>technical collaboration</Link>{' '}and enabling post-incident analysis of decisions.</>
            : <>Os artefatos operacionalizam os princípios de simplicidade, transparência e rastreabilidade do MergeTrace. Eles viabilizam mecanismos de explicabilidade e auditoria — apontados pelos especialistas (P1–P8) como essenciais para mitigar riscos entre equipes, apoiar{' '}<Link to="/guideline-merge/docs/welcome/roles" style={{ color: '#1D9E75', fontWeight: 600 }}>colaboração técnica</Link>{' '}e permitir análise posterior de incidentes e decisões.</>}
        </p>
      </div>
    </div>
  );
