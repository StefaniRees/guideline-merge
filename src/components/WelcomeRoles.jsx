...
export default function WelcomeRoles() {
  ...
  const roles = [
    {
      ...
      avatarBg: '#E1F5EE', avatarColor: '#0F6E56', color: '#639922', bg: '#EAF3DE', border: '#97C459', href: '/guideline-merge/docs/roles/developer-principal',
      ...
      responsibilities: [
        { text: isEN ? 'Implement the task with clear and descriptive commits.' : 'Implementar a tarefa com commits claros e descritivos.', href: '/guideline-merge/docs/guidelines/commit-message' },
        { text: isEN ? 'Open the Pull Request with contextual description and issue link.' : 'Abrir o Pull Request com descrição contextual e vinculação à issue.', href: '/guideline-merge/docs/phases/pull-request' },
        { text: isEN ? 'Fix simple conflicts before review.' : 'Corrigir conflitos simples antes da revisão.', href: '/guideline-merge/docs/phases/merge' },
        { text: isEN ? 'Run unit tests and validate the local build.' : 'Executar testes unitários e validar a build local.', href: '/guideline-merge/docs/guidelines/ci-cd-quality-gates' },
        { text: isEN ? 'Interact with Developer B during collaborative adjustments.' : 'Interagir com Developer B durante ajustes colaborativos.', href: '/guideline-merge/docs/roles/developer-integrador' },
      ],
      artifacts: [
        { text: isEN ? 'Activity branch (feature/ or bugfix/)' : 'Branch de atividade (feature/ ou bugfix/)', href: '/guideline-merge/docs/artifacts/branch-atividade' },
        { text: isEN ? 'Initial Pull Request' : 'Pull Request inicial', href: '/guideline-merge/docs/artifacts/pull-request' },
        { text: isEN ? 'Standardized commits and build logs' : 'Commits padronizados e logs de build', href: '/guideline-merge/docs/artifacts/commits-padronizados' },
      ],
    ...
    },
    {
      ...
      avatarBg: '#E6F1FB', avatarColor: '#185FA5', color: '#378ADD', bg: '#E6F1FB', border: '#85B7EB', href: '/guideline-merge/docs/roles/developer-integrador',
      ...
      responsibilities: [
        { text: isEN ? 'Review the PR created by Developer A, suggesting improvements.' : 'Revisar o PR criado pelo Developer A, sugerindo melhorias.', href: '/guideline-merge/docs/phases/code-review' },
        { text: isEN ? 'Integrate dependencies or related new modules.' : 'Integrar dependências ou novos módulos relacionados.', href: '/guideline-merge/docs/phases/merge' },
        { text: isEN ? 'Support conflict resolution between branches.' : 'Apoiar resolução de conflitos entre branches.', href: '/guideline-merge/docs/guidelines/conflict-resolution-procedure' },
        { text: isEN ? 'Monitor integration test execution and CI/CD.' : 'Acompanhar execução de testes de integração e CI/CD.', href: '/guideline-merge/docs/guidelines/ci-cd-quality-gates' },
        { text: isEN ? 'Communicate blockers or risks to the Mediator.' : 'Comunicar bloqueios ou riscos ao Mediador.', href: '/guideline-merge/docs/roles/mediador' },
      ],
      artifacts: [
        { text: isEN ? 'Complementary commits' : 'Commits complementares', href: '/guideline-merge/docs/artifacts/commits-padronizados' },
        { text: isEN ? 'Resolved conflict history' : 'Histórico de conflitos resolvidos', href: '/guideline-merge/docs/artifacts/historico-conflitos' },
        { text: isEN ? 'Integration pipeline reports' : 'Relatórios de pipeline de integração', href: '/guideline-merge/docs/artifacts/relatorio-build-ci' },
      ],
    ...
    },
    {
      ...
      avatarBg: '#EEEDFE', avatarColor: '#534AB7', color: '#7F77DD', bg: '#EEEDFE', border: '#AFA9EC', href: '/guideline-merge/docs/roles/mediador',
      ...
      responsibilities: [
        { text: isEN ? 'Analyze conflict causes and guide collaborative corrections.' : 'Analisar causas de conflitos e orientar correções colaborativas.', href: '/guideline-merge/docs/guidelines/conflict-resolution-procedure' },
        { text: isEN ? 'Mediate complex reviews between Developer A and Developer B.' : 'Intermediar revisões complexas entre Developer A e Developer B.', href: '/guideline-merge/docs/phases/code-review' },
        { text: isEN ? 'Validate acceptance criteria and merge traceability.' : 'Validar critérios de aceitação e rastreabilidade dos merges.', href: '/guideline-merge/docs/guidelines/approval-policy' },
        { text: isEN ? 'Ensure adherence to Merge Guidelines and Quality Gates.' : 'Garantir aderência aos Merge Guidelines e Quality Gates.', href: '/guideline-merge/docs/guidelines' },
        { text: isEN ? 'Execute the Final Merge after approvals.' : 'Executar o Merge Final após aprovações.', href: '/guideline-merge/docs/phases/merge' },
        { text: isEN ? 'Communicate resolution results and learning to the team.' : 'Comunicar ao time resultados de resolução e aprendizado.', href: '/guideline-merge/docs/artifacts/historico-conflitos' },
      ],
      artifacts: [
        { text: isEN ? 'Resolved conflict history' : 'Histórico de conflitos resolvidos', href: '/guideline-merge/docs/artifacts/historico-conflitos' },
        { text: isEN ? 'Decision records (issues, comments, PRs)' : 'Registro de decisões (issues, comentários, PRs)', href: '/guideline-merge/docs/artifacts/registro-decisao' },
        { text: isEN ? 'Release Notes and incident records' : 'Release Notes e registro de incidentes', href: '/guideline-merge/docs/artifacts/release-notes' },
      ],
  ...
  const openupCards = [
    { role: 'Developer A', roleHref: '/guideline-merge/docs/roles/developer-principal', openup: 'Implementer', desc: isEN ? 'Develops the functional increment and ensures technical conformance.' : 'Elabora o incremento funcional e garante conformidade técnica.', color: '#639922', bg: '#EAF3DE', border: '#97C459' },
    { role: 'Developer B', roleHref: '/guideline-merge/docs/roles/developer-integrador', openup: 'Reviewer', desc: isEN ? 'Continuously verifies, validates, and improves the integrated work.' : 'Verifica, valida e aprimora continuamente o trabalho integrado.', color: '#378ADD', bg: '#E6F1FB', border: '#85B7EB' },
    { role: isEN ? 'Mediator' : 'Mediador', roleHref: '/guideline-merge/docs/roles/mediador', openup: 'Integrator', desc: isEN ? 'Integrates artifacts, coordinates flow, and mitigates merge risks.' : 'Integra artefatos, coordena o fluxo e mitiga riscos de merge.', color: '#7F77DD', bg: '#EEEDFE', border: '#AFA9EC' },
  ];

  const interactions = [
    { step: isEN ? 'Branch creation and implementation' : 'Criação da branch e implementação', stepHref: '/guideline-merge/docs/phases/planejamento', da: isEN ? 'Responsible' : 'Responsável', db: isEN ? 'Monitors' : 'Acompanha', md: '—' },
    { step: isEN ? 'Pull Request opening' : 'Abertura do Pull Request', stepHref: '/guideline-merge/docs/phases/pull-request', da: isEN ? 'Responsible' : 'Responsável', db: isEN ? 'Reviewer' : 'Revisor', md: isEN ? 'Supervises' : 'Supervisiona' },
    { step: isEN ? 'Simple conflict resolution' : 'Resolução de conflitos simples', stepHref: '/guideline-merge/docs/guidelines/conflict-resolution-procedure', da: isEN ? 'Fixes locally' : 'Corrige localmente', db: isEN ? 'Supports' : 'Apoia', md: '—' },
    { step: isEN ? 'Code Review and staging' : 'Code Review e homologação', stepHref: '/guideline-merge/docs/phases/code-review', da: '—', db: isEN ? 'Validates in staging' : 'Valida em homologação', md: isEN ? 'Executes review' : 'Executa revisão' },
    { step: isEN ? 'Complex conflicts / technical decision' : 'Conflitos complexos / decisão técnica', stepHref: '/guideline-merge/docs/guidelines/conflict-resolution-procedure', da: '—', db: isEN ? 'Supports' : 'Apoia', md: isEN ? 'Acts as mediator' : 'Atua como mediador' },
    { step: isEN ? 'Final Merge and release' : 'Merge Final e liberação', stepHref: '/guideline-merge/docs/phases/merge', da: '—', db: '—', md: isEN ? 'Responsible' : 'Responsável' },
  ];
    ...
    <div style={{ padding: '0 0 2rem' }}>
      ...
      <div style={{ background: '#f0faf6', borderLeft: '3px solid #1D9E75', borderRadius: '0 8px 8px 0', padding: '14px 16px', marginBottom: '32px' }}>
        <p style={{ fontSize: '14px', color: '#374151', lineHeight: 1.7, margin: 0 }}>
          {isEN
            ? <>MergeTrace defines <strong>3 collaborative roles</strong> for the merge flow, adapted from the Open Unified Process (OpenUP). The merge works as a <strong>dual authorship (A+B)</strong> supported by a third specialized agent for technical decision-making and coordination — the <Link to="/guideline-merge/docs/roles/mediador" style={{ color: '#7F77DD', fontWeight: 600 }}>Mediator</Link>. The roles <strong>are not hierarchical</strong> — all collaborate within the same continuous flow, with shared responsibility and distributed risk.</>
            : <>O MergeTrace define <strong>3 papéis colaborativos</strong> para o fluxo de merge, adaptados do Open Unified Process (OpenUP). O merge funciona como uma <strong>dupla autoria (A+B)</strong> com suporte de um terceiro agente especializado em tomada de decisão técnica e coordenação — o <Link to="/guideline-merge/docs/roles/mediador" style={{ color: '#7F77DD', fontWeight: 600 }}>Mediador</Link>. Os papéis <strong>não são hierárquicos</strong> — todos colaboram dentro de um mesmo fluxo contínuo, com responsabilidade compartilhada e risco distribuído.</>}
        </p>
      </div>
      ...
      <div style={{ background: '#fafafa', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '14px 16px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
        ...
        <p style={{ fontSize: '13px', color: '#374151', lineHeight: 1.6, margin: 0 }}>
          {isEN
            ? <>MergeTrace records events and role interactions throughout the flow — <Link to="/guideline-merge/docs/artifacts/pull-request" style={{ color: '#1D9E75', fontWeight: 600 }}>PRs</Link>, <Link to="/guideline-merge/docs/phases/merge" style={{ color: '#1D9E75', fontWeight: 600 }}>merges</Link>, <Link to="/guideline-merge/docs/artifacts/relatorio-build-ci" style={{ color: '#1D9E75', fontWeight: 600 }}>builds</Link>, and <Link to="/guideline-merge/docs/artifacts/historico-conflitos" style={{ color: '#1D9E75', fontWeight: 600 }}>logs</Link>. Traceability allows identifying <strong>who participated in each technical decision and resolved conflict</strong>, supporting governance and organizational learning.</>
            : <>O MergeTrace registra eventos e interações entre papéis ao longo do fluxo — <Link to="/guideline-merge/docs/artifacts/pull-request" style={{ color: '#1D9E75', fontWeight: 600 }}>PRs</Link>, <Link to="/guideline-merge/docs/phases/merge" style={{ color: '#1D9E75', fontWeight: 600 }}>merges</Link>, <Link to="/guideline-merge/docs/artifacts/relatorio-build-ci" style={{ color: '#1D9E75', fontWeight: 600 }}>builds</Link> e <Link to="/guideline-merge/docs/artifacts/historico-conflitos" style={{ color: '#1D9E75', fontWeight: 600 }}>logs</Link>. A rastreabilidade permite identificar <strong>quem participou de cada decisão técnica e conflito resolvido</strong>, apoiando governança e aprendizado organizacional.</>}
        </p>
      </div>
    </div>
  );
