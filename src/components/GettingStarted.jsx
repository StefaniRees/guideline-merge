...
export default function GettingStarted() {
  ...
  const structure = [
    { label: isEN ? 'Phases' : 'Fases', desc: isEN ? '8 sequential operational phases: from planning to rollback.' : '8 fases operacionais sequenciais: do planejamento ao rollback.', color: '#BA7517', bg: '#FAEEDA', href: '/guideline-merge/docs/phases', icon: (c) => (<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M3 11h4l3-5 3 10 3-5h3" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>) },
    { label: isEN ? 'Roles' : 'Papéis', desc: isEN ? 'Developer A, Developer B, and Mediator — explicit responsibilities.' : 'Developer A, Developer B e Mediador — responsabilidades explícitas.', color: '#7F77DD', bg: '#EEEDFE', href: '/guideline-merge/docs/roles', icon: (c) => (<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><circle cx="8" cy="8" r="3" stroke={c} strokeWidth="1.5"/><circle cx="14" cy="8" r="3" stroke={c} strokeWidth="1.5"/><path d="M2 18c0-2.5 2-4 6-4M10 18c0-2.5 2-4 6-4" stroke={c} strokeWidth="1.5" strokeLinecap="round"/></svg>) },
    { label: isEN ? 'Activities' : 'Atividades', desc: isEN ? '8 executable activities with inputs, outputs, and quality gates.' : '8 atividades executáveis com entradas, saídas e quality gates.', color: '#639922', bg: '#EAF3DE', href: '/guideline-merge/docs/activities', icon: (c) => (<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><rect x="3" y="3" width="16" height="16" rx="3" stroke={c} strokeWidth="1.5"/><path d="M7 8h8M7 11h8M7 14h5" stroke={c} strokeWidth="1.5" strokeLinecap="round"/></svg>) },
    { label: isEN ? 'Artifacts' : 'Artefatos', desc: isEN ? '10 traceable artifacts connecting phases, decisions, and results.' : '10 artefatos rastreáveis que conectam fases, decisões e resultados.', color: '#993556', bg: '#FBEAF0', href: '/guideline-merge/docs/artifacts', icon: (c) => (<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><rect x="4" y="3" width="14" height="16" rx="2" stroke={c} strokeWidth="1.5"/><path d="M7 8h8M7 11h8M7 14h5" stroke={c} strokeWidth="1.5" strokeLinecap="round"/></svg>) },
    { label: 'Guidelines', desc: isEN ? '11 quality gates and process governance guidelines.' : '11 quality gates e diretrizes de governança do processo.', color: '#0F6E56', bg: '#E1F5EE', href: '/guideline-merge/docs/guidelines', icon: (c) => (<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><circle cx="11" cy="11" r="8" stroke={c} strokeWidth="1.5"/><path d="M11 7v5l3 3" stroke={c} strokeWidth="1.5" strokeLinecap="round"/></svg>) },
    { label: 'Templates', desc: isEN ? 'Operational checklists per phase to standardize practices.' : 'Checklists operacionais por fase para padronizar práticas.', color: '#378ADD', bg: '#E6F1FB', href: '/guideline-merge/docs/templates', icon: (c) => (<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><rect x="3" y="3" width="16" height="16" rx="3" stroke={c} strokeWidth="1.5"/><path d="M7 8l2 2 4-4" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/><path d="M7 13l2 2 4-4" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>) },
  ];

  const steps = [
    { n: '1', color: '#639922', bg: '#EAF3DE', title: isEN ? 'Understand the principles' : 'Entenda os princípios', desc: isEN ? 'Read Core Principles and understand the O1–O4 objectives of the guideline.' : 'Leia Core Principles e compreenda os objetivos O1–O4 do guideline.', href: '/guideline-merge/docs/welcome/core-principles' },
    { n: '2', color: '#378ADD', bg: '#E6F1FB', title: isEN ? 'Learn the roles' : 'Conheça os papéis', desc: isEN ? 'Identify who will be Developer A, Developer B, and Mediator in your context.' : 'Identifique quem será Developer A, Developer B e Mediador no seu contexto.', href: '/guideline-merge/docs/roles' },
    { n: '3', color: '#BA7517', bg: '#FAEEDA', title: isEN ? 'Walk through the phases' : 'Percorra as fases', desc: isEN ? 'Explore the 8 phases and their quality gates to understand the complete flow.' : 'Explore as 8 fases e seus quality gates para entender o fluxo completo.', href: '/guideline-merge/docs/phases' },
    { n: '4', color: '#7F77DD', bg: '#EEEDFE', title: isEN ? 'Adopt the templates' : 'Adote os templates', desc: isEN ? 'Use the per-phase checklists as a practical entry point.' : 'Use os checklists por fase como ponto de entrada prático.', href: '/guideline-merge/docs/templates' },
  ];
