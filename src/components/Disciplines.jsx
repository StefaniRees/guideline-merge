  ...
  const disciplines = [
    {
      ...
      activities: [
        { label: isEN ? 'Create Branch' : 'Criar Branch', href: '/guideline-merge/docs/activities/criar-branch' },
        { label: isEN ? 'Implement Changes' : 'Implementar Mudanças', href: '/guideline-merge/docs/activities/implementar-mudancas' },
      ],
      artifacts: [
        { label: isEN ? 'Activity Branch' : 'Branch de Atividade', href: '/guideline-merge/docs/artifacts/branch-atividade' },
        { label: isEN ? 'Standardized Commits' : 'Commits Padronizados', href: '/guideline-merge/docs/artifacts/commits-padronizados' },
      ],
      guidelines: [
    ...
    },
    {
      ...
      activities: [
        ...
        { label: isEN ? 'Validate in Staging (QA)' : 'Validar em Homologação (QA)', href: '/guideline-merge/docs/activities/qa-homologacao' },
      ],
      artifacts: [
        { label: isEN ? 'Review Checklist' : 'Checklist de Revisão', href: '/guideline-merge/docs/artifacts/checklist-revisao' },
        { label: isEN ? 'QA Report' : 'Relatório de QA', href: '/guideline-merge/docs/artifacts/relatorio-qa' },
        { label: isEN ? 'Decision Record' : 'Registro de Decisão', href: '/guideline-merge/docs/artifacts/registro-decisao' },
      ],
      guidelines: [
      ...
      roles: [
        { label: isEN ? 'Mediator' : 'Mediador', href: '/guideline-merge/docs/roles/mediador', color: '#7F77DD', bg: '#EEEDFE', border: '#AFA9EC' },
        { label: 'Developer B', href: '/guideline-merge/docs/roles/developer-integrador', color: '#378ADD', bg: '#E6F1FB', border: '#85B7EB' },
      ],
    ...
    },
    {
      ...
      activities: [
        { label: isEN ? 'Run Build and Tests (CI/CD)' : 'Executar Build e Testes (CI/CD)', href: '/guideline-merge/docs/activities/build-ci-cd' },
        { label: isEN ? 'Validate in Staging (QA)' : 'Validar em Homologação (QA)', href: '/guideline-merge/docs/activities/qa-homologacao' },
      ],
      artifacts: [
        { label: isEN ? 'Build/CI Report' : 'Relatório de Build/CI', href: '/guideline-merge/docs/artifacts/relatorio-build-ci' },
        { label: isEN ? 'QA Report' : 'Relatório de QA', href: '/guideline-merge/docs/artifacts/relatorio-qa' },
      ],
      guidelines: [
    ...
    },
    {
      ...
      activities: [
        { label: isEN ? 'Run Build and Tests (CI/CD)' : 'Executar Build e Testes (CI/CD)', href: '/guideline-merge/docs/activities/build-ci-cd' },
        { label: isEN ? 'Final Merge' : 'Merge Final', href: '/guideline-merge/docs/activities/merge-final' },
      ],
      artifacts: [
        { label: isEN ? 'Build/CI Report' : 'Relatório de Build/CI', href: '/guideline-merge/docs/artifacts/relatorio-build-ci' },
    ...
    },
    {
      ...
      activities: [
        { label: isEN ? 'Resolve Merge Conflicts' : 'Resolver Conflitos de Merge', href: '/guideline-merge/docs/activities/resolver-conflitos' },
        { label: isEN ? 'Final Merge' : 'Merge Final', href: '/guideline-merge/docs/activities/merge-final' },
      ],
      artifacts: [
        { label: isEN ? 'Conflict History' : 'Histórico de Conflitos', href: '/guideline-merge/docs/artifacts/historico-conflitos' },
        { label: isEN ? 'Decision Record' : 'Registro de Decisão', href: '/guideline-merge/docs/artifacts/registro-decisao' },
        { label: 'Release Notes', href: '/guideline-merge/docs/artifacts/release-notes' },
      ],
      guidelines: [
    ...
    <div style={{ padding: '0 0 2rem' }}>
      ...
      <div style={{ background: '#f0faf6', borderLeft: '3px solid #1D9E75', borderRadius: '0 8px 8px 0', padding: '14px 16px', marginBottom: '32px' }}>
        <p style={{ fontSize: '14px', color: '#374151', lineHeight: 1.7, margin: 0 }}>
          {isEN ? <>The <strong>MergeTrace disciplines</strong> group{' '}<Link to="/guideline-merge/docs/activities" style={{ color: '#1D9E75', fontWeight: 600 }}>activities</Link> and{' '}<Link to="/guideline-merge/docs/roles" style={{ color: '#1D9E75', fontWeight: 600 }}>roles</Link> that share a common technical or collaborative focus. As in OpenUP, disciplines <strong>are not isolated compartments</strong> — they overlap and interact constantly throughout the integration cycle, connecting developers, reviewers, and mediators around common goals. Click each discipline to see details.</> : <>As <strong>disciplinas do MergeTrace</strong> agrupam{' '}<Link to="/guideline-merge/docs/activities" style={{ color: '#1D9E75', fontWeight: 600 }}>atividades</Link> e{' '}<Link to="/guideline-merge/docs/roles" style={{ color: '#1D9E75', fontWeight: 600 }}>papéis</Link> que compartilham um mesmo foco técnico ou colaborativo. Assim como no OpenUP, as disciplinas <strong>não são compartimentos isolados</strong> — elas se sobrepõem e interagem constantemente ao longo do ciclo de integração, conectando desenvolvedores, revisores e mediadores em torno de metas comuns. Clique em cada disciplina para ver detalhes.</>}
        </p>
      </div>
      ...
      <div style={{ background: '#fafafa', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '14px 16px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
        ...
        <p style={{ fontSize: '13px', color: '#374151', lineHeight: 1.6, margin: 0 }}>
          {isEN ? <>Each discipline can be represented by a reference workflow that illustrates typical{' '}<Link to="/guideline-merge/docs/activities" style={{ color: '#1D9E75', fontWeight: 600 }}>activities</Link>, dependencies between{' '}<Link to="/guideline-merge/docs/roles" style={{ color: '#1D9E75', fontWeight: 600 }}>roles</Link>, and{' '}<Link to="/guideline-merge/docs/artifacts" style={{ color: '#1D9E75', fontWeight: 600 }}>artifacts</Link> produced. These flows serve as educational guides and reference for continuous team improvement.</> : <>Cada disciplina pode ser representada por um workflow de referência que ilustra as{' '}<Link to="/guideline-merge/docs/activities" style={{ color: '#1D9E75', fontWeight: 600 }}>atividades</Link> típicas, as dependências entre{' '}<Link to="/guideline-merge/docs/roles" style={{ color: '#1D9E75', fontWeight: 600 }}>papéis</Link> e os{' '}<Link to="/guideline-merge/docs/artifacts" style={{ color: '#1D9E75', fontWeight: 600 }}>artefatos</Link> gerados. Esses fluxos servem como guias educacionais e referência para aprimoramento contínuo do time.</>}
        </p>
      </div>
    </div>
  );
